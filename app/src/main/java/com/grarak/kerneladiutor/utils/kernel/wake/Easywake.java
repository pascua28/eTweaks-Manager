 ...
 */
public class Easywake {

    private static Easywake sInstance;

    public static Easywake getInstance() {
        if (sInstance == null) {
            sInstance = new Easywake();
        }
        return sInstance;
    }

    private static final String EASY_WAKEUP = "/sys/devices/platform/huawei_touch/easy_wakeup_gesture";

    private final List<String> mFiles = new ArrayList<>();

    {
        mFiles.add(EASY_WAKEUP);
    }
    ...
    public void set(int value, Context context) {
        if (FILE.equals(EASY_WAKEUP)) {
            run(Control.write(value == 0 ? "0" : "1", FILE), FILE, context);
        } else {
    ...
    public int get() {
        if (FILE.equals(EASY_WAKEUP)) {
            return Utils.readFile(FILE).equals("0x0000") ? 0 : 1;
        } else {
